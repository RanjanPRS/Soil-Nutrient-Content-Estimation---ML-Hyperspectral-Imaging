...
# Filter dataset for N categories
n_column = 'N_Output'  # Update to the column representing N categories
n_categories = ['LOW', 'Low1']
...
def optimize_for_n_category(subset, n_values, num_top_bands=10):
    if len(subset) < 2:
    ...
    def objective_function(weights):
        ...
        correlation, _ = pearsonr(weighted_sum, n_values)
...
# Apply the process for each N category
results = {}
for category in n_categories:
    subset = df[df[n_column] == category]
    n_values = np.array(subset['N'], dtype=float)  # Extract N values for correlation
    results[category] = optimize_for_n_category(subset, n_values)

# Print results for each N category
