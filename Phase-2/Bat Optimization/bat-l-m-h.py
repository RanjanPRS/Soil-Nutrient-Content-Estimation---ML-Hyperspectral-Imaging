...
# Filter dataset for OC categories
oc_column = 'OC_Output'
oc_categories = ['LOW', 'Medium', 'High']
...
def optimize_for_oc_category(subset, oc_values, num_top_bands=10):
    if len(subset) < 2:
    ...
    def objective_function(weights):
        ...
        correlation, _ = pearsonr(weighted_sum, oc_values)
...
# Apply the process for each OC category
results = {}
for category in oc_categories:
    subset = df[df[oc_column] == category]
    oc_values = np.array(subset['OC'], dtype=float)  # Extract OC values for correlation
    results[category] = optimize_for_oc_category(subset, oc_values)

# Print results for each OC category
