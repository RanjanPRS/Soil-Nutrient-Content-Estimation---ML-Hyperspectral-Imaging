...
# Filter dataset for K categories
k_column = 'K_Output'  # Update to the column representing K categories
k_categories = ['High', 'Medium']
...
def optimize_for_k_category(subset, k_values, num_top_bands=10):
    if len(subset) < 2:
    ...
    def objective_function(weights):
        ...
        correlation, _ = pearsonr(weighted_sum, k_values)
...
# Apply the process for each K category
results = {}
for category in k_categories:
    subset = df[df[k_column] == category]
    k_values = np.array(subset['K'], dtype=float)  # Extract K values for correlation
    results[category] = optimize_for_k_category(subset, k_values)

# Print results for each K category
